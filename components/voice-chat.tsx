...
export default function VoiceChat() {
  ...
  const springyGradient = hsl.to(
    (h: number, s: number, l: number) =>
      `radial-gradient(hsl(${h}, ${s * 0.7}%, ${l}%), hsl(${h},${s * 0.4}%, ${l * 0.2}%))`
  );
  return (
    <div className="fixed top-0 left-0 w-screen h-screen overflow-hidden flex items-center justify-center z-[1000]">
      <aw.div
 ...
 */
function MagicMarbleMaterial({
  step,
  ...props
}: { step: number } & THREE.MeshStandardMaterialParameters) {
